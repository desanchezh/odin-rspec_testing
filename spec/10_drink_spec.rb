...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:inka_kola) { described_class.new('inka kola') }

      # remove the 'x' before running this test
      it 'is inka kola' do
        expect(inka_kola.type).to eq('inka kola')
      end
      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(inka_kola.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:inka_kola) { described_class.new('inka kola', 20) }
      # remove the 'x' before running this test
      it 'is full' do
        expect(inka_kola.ounces).to(be >= 16)
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:inka_kola) { described_class.new('inka kola', 8) }
