...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:number_guessed) { double('number_guessing', value: 12) }
      subject(:game_won) { described_class.new(0, 20, number_guessed, 12) }
    ...
    context 'when guess and random_number are not equal' do
      let(:number_guessing) { double('number_guessing', value: 12) }
      subject(:game_continues) { described_class.new(0, 20, number_guessing, 11) }
      it 'is not game over' do
        expect(game_continues.game_over?).not_to be true
      end
    end
  end
